...
export class GamesService {
  ...
  public static update(game: Game): Game | undefined {
    ...
    if (
      gamesDB[index].status === EGameStatus.FINISHED ||
      gamesDB[index].status === EGameStatus.CANCELLED
    ) {
      return undefined;
    }
    ...
    if (
      ...
      game.status = EGameStatus.SCHEDULED;
    }
  ...
  public static updateGameStatus(
    id: number,
    status: EGameStatus,
  ): Game | undefined {
    const gamesDB = this.readGamesDB();
    ...
    if (
      ...
      if (
        gamesDB[index].fieldId &&
        gamesDB[index].refereeId &&
        gamesDB[index].homeTeamId &&
        gamesDB[index].awayTeamId &&
        gamesDB[index].homeScore === 0 &&
        gamesDB[index].awayScore === 0
      ) {
        isTransitionAllowed = true;
      }
    }
    ...
    if (
      gamesDB[index].status === EGameStatus.STARTED &&
